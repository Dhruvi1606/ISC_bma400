...
void delay_ms(uint32_t period)
{
    k_msleep(period);/* Wait for a period amount of ms*/
}
...
int8_t i2c_reg_write(void *intf_ptr, uint8_t i2c_addr, uint8_t reg_addr, uint8_t *reg_data, uint16_t length)
{
    ...
    /* Prepare the buffer with register address followed by the data */
    buf[0] = reg_addr;
    memcpy(&buf[1], reg_data, length);

    /* Write to the I2C device */
    ret = i2c_write(i2c_dev, buf, sizeof(buf), i2c_addr);
    if (ret != 0) {
...
int8_t i2c_reg_read(void *intf_ptr, uint8_t i2c_addr, uint8_t reg_addr, uint8_t *reg_data, uint16_t length)
{
    ...
    ret = i2c_write(i2c_dev, &reg_addr, 1, i2c_addr);
    if (ret != 0) {
        return -1; // Communication failure
    }
